...
/*
 * KiCad schematic (.kicad_sch) viewer.
 *
 * Fetches the raw schematic, renders it to SVG with the self-contained
 * kicadSchRenderer, and presents it in a pan/zoom canvas that mimics opening
 * the sheet in KiCad's Eeschema. Falls back gracefully to a readable message
 * plus the raw source for legacy .sch files or anything it cannot draw.
 */
...
export default function KicadSchematicViewer({ fileUrl }) {
  ...
  const applyView = useCallback(() => {
    ...
    forceTick((t) => t + 1); // refresh zoom readout
  }, []);
  ...
  if (state.status === 'loading') {
    return shell(<p className="text-muted p-3">Loading schematic…</p>);
  }
    ...
    return shell(
      <div style={{ padding: '1rem', color: '#ddd', background: '#1b1e22', minHeight: '600px' }}>
        <p style={{ color: state.legacy ? '#e0b040' : '#e06060', fontWeight: 600 }}>
          {state.legacy ? '⚠ Legacy schematic format' : '⚠ Cannot render schematic'}
        </p>
  ...
  return shell(
    <div style={{ background: '#f4f4ee', position: 'relative', height: '600px' }}>
      {/* toolbar */}
      <div
        style={{
          position: 'absolute',
      ...
      >
        <span style={{ color: '#9aa', fontSize: '0.75rem', marginRight: 2 }}>
          {st.symbols || 0} sym · {st.wires || 0} wires{st.sheets ? ` · ${st.sheets} sheets` : ''}
        </span>
      ...
      >
        ...
        >
          <rect x={v.x - v.w} y={v.y - v.h} width={v.w * 3} height={v.h * 3} fill={state.background || '#ffffff'} />
