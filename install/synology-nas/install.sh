#!/bin/bash
# Mini-PLM Install Script for Synology NAS
# Usage: bash install.sh

set -e

DEFAULT_DIR="/volume1/docker/mini-plm"
...
# Choose where everything lives. This directory holds your uploaded files, so on a
# NAS put it on whichever volume has the room: /volume2/docker/mini-plm and so on.
...
if ! mkdir -p "$INSTALL_DIR/mpp_files" "$INSTALL_DIR/nginx/conf" 2>/dev/null; then
    echo "    Cannot create $INSTALL_DIR. Check that the volume exists and you have permission."
    exit 1
fi
...
echo "=== Done! ==="
echo ""
echo "There is no app to launch. Mini-PLM runs in the background on the NAS and"
echo "you use it in a browser from any machine on the network."
echo ""
echo "    Open:  http://$NAS_IP:$PORT"
echo ""
echo "The first visit shows a setup wizard for creating your admin account."
echo "It restarts with the NAS, so you do not need to run this again."
echo ""
